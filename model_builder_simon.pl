...
while(1){
	...
	if($para[0] != -1){
		printf "\nrange= %u-%u, b/a= %.2f, sum=%u\nxmin= %u, xmax= %f, a= %.3f, KS=%.5f, p-value=%.2f\n", 
			$para[4],$para[5],$para[1]/$para[0],$para[6],$para[0],$para[1]+0,$para[2],$para[3], $para[7];
		printf OUTPUT "\nrange= %u-%u(%u), b/a= %.2f, sum=%u\nxmin= %f, xmax= %f, a= %.3f, KS=%.5f, p-value=%.2f\n", 
			$para[4],$para[5],$para[5]-$para[4]+1,$para[1]/$para[0],$para[6],$para[0],$para[1]+0,$para[2],$para[3], $para[7];
		push @{$modArr}, \@para;
	}
	$B = $para[4];
...
sub build_mod{
	...
	my ($xdata,$ydata, $pdata) = ([],[], []); #freqs, D-values
	for my $n(reverse 0..$u_limit){
		my $_xmin=$freqarr->[$n];
		#print "_xmin=$_xmin\t";
		$sum+=$countarr->[$n];
		$lnsum+=$countarr->[$n]*log $_xmin;
		my $_alpha=find_alpha($_xmin, $xmax, $sum, $lnsum);  #TODO don't know the MLE of alpha with upper bound
		next if ($n == $u_limit);
		#print "$lnsum, $sum, ",log($_xmin)," \n";
		$_alpha = 1+$sum/($lnsum-$sum*log($_xmin)) if($cont);
		if(!$cont && zeta($_xmin, $xmax, $_alpha)==0){ next;}
		if($cont && $_xmin**(1-$_alpha)-$xmax**(1-$_alpha) ==0) {next;}
		
		my $D ;
		if(!$cont){$D= KS($freqarr, $countarr, $n, $u_limit, $_alpha, $sum, !$upperbound);}
		else{$D = KS_cont($freqarr, $countarr, $n, $u_limit, $_alpha, $sum, !$upperbound);}
		
		push @$xdata, $_xmin;
		push @$ydata, $D;
		
		#calculating p-value
		my ($p, $numofdata)=(0,100);
		if ($earlytermination && $freepass>0){$freepass--;$p=0;}
		else{
			for (1..$numofdata){
				my $sdata;
				$sdata = plrand_cont($_xmin, $xmax,$_alpha,$sum) if ($cont);
				$sdata = plrand($_xmin, $xmax, $_alpha, $sum) if (! $cont);
				my %f2c;
				for (@{$sdata}){
				...
				my $sD;
				$sD=KS_cont(\@sfreqs,\@scounts,0,$#sfreqs,$_alpha,$ssum,!$upperbound,$D) if ($cont);
				$sD=KS(\@sfreqs, \@scounts,0,$#sfreqs, $_alpha, $ssum,!$upperbound,$D) if(!$cont);
				$p++ if ($D<$sD);
	...
	if($l_limit != $u_limit)#found a probable model
	{
		push @{$xdatas}, $xdata;
		push @{$ydatas}, $ydata;
		push @{$pdatas}, $pdata;
	}
	else{$xmin = -1;}#indicating that no possible model found with this xmax
